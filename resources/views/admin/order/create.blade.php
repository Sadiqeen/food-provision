...
@section('content')
    <div class="container">
        <h3 class="my-3 text-uppercase font-weight-bold d-flex">
            ...
            <a type="button" href="{{ route('admin.order.upload') }}"  class="btn btn-secondary mr-2">{{ __('Import Order') }}</a>
            <a type="button" href="{{ route( auth()->user()->position . '.order.cart') }}"
               class="btn btn-success">{{ __('Cart') }}</a>
        </h3>
        <div class="card">
            <div class="card-body">
                {{-- Top select option --}}
...
@push('script')
    ...
    <script>
        ...
        const updateCart = function (id, url) {
            let quantity = parseFloat($('#product-quantity-' + id).val())
            if (!quantity) {
                quantity = 0
                $('#product-quantity-' + id).val(0)
            }
